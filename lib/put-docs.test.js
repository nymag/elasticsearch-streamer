...
describe('get-docs', function () {
  ...
  it ('sends bulk request to index items from source stream', function (done) {
  ...
  it ('if elastic endpoint includes index, sets _index of each bulk item', function (done) {
    fn(h(mockDocs), 'http://foo.com/bar').toArray(() => {
      ...
      expect(bulkOps[0].index._index).to.equal('bar');
      expect(bulkOps[2].index._index).to.equal('bar');
      done();
    });
  });
});
