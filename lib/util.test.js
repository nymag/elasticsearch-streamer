...
describe('scrollStream', function () {
  ...
  it ('stops and throws error if search request errors', function (done) {
    client.search.onCall(0).rejects();

    fn(client)
      .collect()
      .toPromise(Promise)
      .then(() => {
        done(new Error('Expected function to error'));
      })
      .catch(() => {
        expect(client.scroll.getCalls().length).to.equal(0);
        done();
      })
      .catch(done);
  });

  it ('stops and throws error if scroll request errors', function (done) {
    client.scroll.onCall(0).rejects();
    ...
    fn(client)
      ...
      .catch(() => {
        expect(client.scroll.getCalls().length).to.equal(1);
        done();
      })
      .catch(done);
  });
